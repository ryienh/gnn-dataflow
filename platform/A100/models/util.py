...
def get_children(df, op_id):
    ...
    return df[
        df[UID].isin(np.concatenate(df[df[UID] == op_id]["cpu_children"].to_numpy()))
    ]
...
def prof_to_df(prof):
    ...
    df["cpu_children"] = df["cpu_children"].apply(
        lambda x: [df[df["FE"].__eq__(c)][UID].item() for c in x]
    )
    #     df["cpu_parent"] = df["cpu_parent"].apply(lambda x: x.id if x is not None else -1)
    df["cpu_parent"] = df["cpu_parent"].apply(
        lambda x: df[df["FE"].__eq__(x)][UID].item() if x is not None else -1
    )
    df["input_shapes"] = df["input_shapes"].astype(str)
    # get operator self time
    df["children_time"] = df.apply(
        lambda x: get_children(df, x[UID])["time_range"].sum(), axis=1
    )
